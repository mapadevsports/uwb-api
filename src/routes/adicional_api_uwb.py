...
@uwb_bp.route('/uwb/data-batch', methods=['POST'])
def receive_uwb_data_batch():
    """
    Novo endpoint para receber array de objetos JSON com dados UWB
    """
    try:
        dados = request.json
        ...
        if not isinstance(dados, list):
            return jsonify({'error': 'Esperado um array de objetos JSON'}), 400
        ...
        if not relatorio_ativo:
            return jsonify({'success': False, 'message': 'Nenhum relatório ativo'}), 400

        processados = []

        for item in dados:
            tag_id = str(item.get('id'))
            range_values = item.get('range', [])

            if not tag_id or not isinstance(range_values, list) or len(range_values) != 8:
            ...
            x, y = trilateracao.processar_distancias(
                ...
                da6=uwb_data.da6,
                da7=uwb_data.da7,
                kx=relatorio_ativo.kx,
                ky=relatorio_ativo.ky
            )

            # Salvar resultado processado
            uwb_processado = UWBDataProcessada(
                tag_number=tag_id,
                x=x,
                y=y,
                criado_em=datetime.utcnow()
            )
            db.session.add(uwb_processado)
