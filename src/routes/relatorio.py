...
def iniciar_relatorio():
    ...
    try:
        ...
        relatorio_ativo = Relatorio.query.filter_by(status='ativo').first()
        ...
        logging.info(f"Novo relatório iniciado: ID {novo_relatorio.id}")
...
def finalizar_relatorio():
    ...
    try:
        ...
        relatorio_ativo.status = 'finalizado'
        relatorio_ativo.atualizado_em = datetime.utcnow()
        
        db.session.commit()
        
        logging.info(f"Relatório finalizado: ID {relatorio_ativo.id}")
...
def historico_relatorios():
    ...
    try:
        ...
        relatorios = Relatorio.query.order_by(Relatorio.criado_em.desc()).limit(limit).all()
...
@relatorio_bp.route('/relatorio/<int:relatorio_id>', methods=['GET'])
...
def health_check_relatorio():
    ...
    try:
        ...
        relatorios_ativos = Relatorio.query.filter_by(status='ativo').count()
        relatorios_finalizados = Relatorio.query.filter_by(status='finalizado').count()
